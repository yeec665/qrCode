...
public class ReedSolomon {
     ...
     */
    private static final int[][] GENERATOR_POLYNOMIAL = {
            ...
            {0, 102, 43, 98, 121, 187, 113, 198, 143, 131, 87, 157, 66}, // 12
    ...
    private static final byte[] TEST_MSG = { // 16
            (byte) 32,
            (byte) 91,
            (byte) 11,
            (byte) 120,
    ...
    public static void test() {
        printBytesDec(divide(TEST_MSG, 10));
    }

    public static void printBytesDec(byte[] bb) {
        for (byte b : bb) {
    ...
    public static void printBytesBin(byte[] bb) {
        for (byte b : bb) {
            for (int i = 0; i < 8; i++) {
                System.out.print((b & 0x80) != 0 ? '1' : '0');
                b <<= 1;
            }
            System.out.print(' ');
        }
    ...
    public static byte[] divide(byte[] msg, int genLength) {
        return divide(msg, getGeneratorPolynomial(genLength));
    }

    public static int[] getGeneratorPolynomial(int len) {
        int index = len - 7;
        if (index >= 0 && index < GENERATOR_POLYNOMIAL.length) {
            int[] out = GENERATOR_POLYNOMIAL[index];
            if (out.length == len + 1) {
                return out;
            }
        }
        throw new IllegalArgumentException("GP : len = " + len);
    }

    public static byte[] divide(byte[] msg, int[] gen) {
        msg = pad(msg, gen);
        int step = firstNonZero(msg);
        int limit = msg.length - gen.length;
        int k = 0;
        while (step <= limit) {
            int m = GaloisField256.logConvert(msg[step]);
            for (int i = 0; i < gen.length; i++) {
                msg[step + i] ^= GaloisField256.baseConvert((gen[i] + m) % 255);
            }
            step = firstNonZero(msg);
        }
        byte[] out = new byte[msg.length - step];
        System.arraycopy(msg, step, out, 0, out.length);
        return out;
    }

    public static byte[] pad(byte[] msg, int[] gen) {
        byte[] nMsg = new byte[msg.length + gen.length - 1];
        System.arraycopy(msg, 0, nMsg, 0, msg.length);
        return nMsg;
    }

    public static int firstNonZero(byte[] b) {
